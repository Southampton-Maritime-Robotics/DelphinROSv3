...
############ MAIN RECKONER LOOP ################################################
    while not rospy.is_shutdown():
        ...
        if delta_t >= dt:
            time_zero = time.time()
        #### GPS DATA ######################################################    
            # reset a reference position and speed in dead reckoning when the GPS is available
            if gpsInfo.fix == 1 and gpsInfo.number_of_satelites >= 5 and cur_depth_demand < 0.5:
                X = gpsInfo.x
                Y = gpsInfo.y
        ...
        #### COMPASS DATA ##################################################
            ...
            depth   = cur_depthInfo.depth_filt
            pitch   = cur_compassInfo.pitch
            roll    = cur_compassInfo.roll
            velZ    = cur_depthInfo.depth_der
