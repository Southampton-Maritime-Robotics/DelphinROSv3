#!/usr/bin/python
# this code lets the motherboard communicate with arduino that is connected to ECS, It will not be used any more an the Maxon board can do this task much better
...
def init_serial():
    ...
    serialPort = serial.Serial(port='/dev/usbArduino', baudrate='57600', timeout=0.1)
...
####    forward
        str = 'faP%d@%d@%d@%d@cs' %(100,100,100,100)
        rospy.loginfo(str)
        serialPort.write(str)
        time.sleep(delaySerial)
####        str = 'faP%d@%d@%d@%d@cs' %(92,92,92,92)
####        rospy.loginfo(str)
####        serialPort.write(str)
####        time.sleep(delaySerial)
####        str = 'faP%d@%d@%d@%d@cs' %(97,97,97,97)
####        rospy.loginfo(str)
####        serialPort.write(str)
####        time.sleep(delaySerial)
####        str = 'faP%d@%d@%d@%d@cs' %(92,92,92,92)
...
####    reverse        
####        str = 'faP%d@%d@%d@%d@cs' %(86,86,86,86)
####        rospy.loginfo(str)
####        serialPort.write(str)
####        time.sleep(delaySerial)
####        str = 'faP%d@%d@%d@%d@cs' %(92,92,92,92)
...
####        serialPort.flushInput()   
        ...
        if serialPort.inWaiting(): # if arduino replied
            ...
            while(timeElapse<timeOut) and not rospy.is_shutdown():
                if serialPort.inWaiting()>0: 
                    ...
                    if _in == '#': # searching for a header '#', and continue if found. otherwise, neglect this data packet
                        ...
                        timeStart = time.time()
                        timeElapse = time.time()-timeStart    
                        while(timeElapse<timeOut) and not rospy.is_shutdown(): # get the rest of a data packet
                            if serialPort.inWaiting()>0:
                                _in = serialPort.read(1)
