...
def EnergyConsumed_callback(data):
    ...
    with open('%s/energyLog.csv' %(dirname), "a") as f:
        try:
            Writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            Writer.writerow(tailList)
        except ValueError:
...
def shutdown():
    ...
    #print 'shutting down'
    pubStatus.publish(nodeID = 10, status = False)
    with open('%s/path.kml' %(dirname), "a") as f:
        try:  
            f.write('</coordinates>\n </LineString>\n </Placemark>\n </kml>\n')
        except ValueError:
...
################################################################################
    ...
    rospy.Subscriber('EnergyConsumed', energy_consumed, EnergyConsumed_callback)
...
#    rospy.Subscriber('water_temp', Float32, temp_callback)
    ...
    # to control a timing for status publishing
    r = rospy.Rate(1/float(dt_status))
