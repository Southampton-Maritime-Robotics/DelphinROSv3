...
#			self.GoToConfig()
	...
	## Request Lat Lon Alt from MTi-G-700
	def ReqLatLonAlt(self): #XXX this section has been added by kantapon XXX
		"""Request Lat Lon Alt from MTi-G-700"""
####		print "printed from the inside of ReqLatLonAlt"
		self.write_ack(MID.ReqLatLonAlt)
...
####		print "printed from the inside of SetOutputConfiguration"
	...
	## Request output configuration #XXX this section has been added by kantapon XXX
	def ReqOutputConfiguration(self):
		self.GoToConfig()
####		print "printed from the inside of ReqOutputConfiguration"
		self.write_ack(MID.OutputConfiguration) # having a same MID as SetOutputConfiguraiton
		self.GoToMeasurement()
...
####		print output['Orientation Data']['Roll']
		return output
		# {'Orientation Data': {'Yaw': 65.52528381347656, 'Roll': -88.75276947021484, 'Pitch': -2.7406132221221924}, 'Temperature': {'Temp': 35.1875}}
...
def find_devices():
	mtdev_list = []
	for port in glob.glob("/dev/usbxsens"): #tty*S*
		try:
...
################################################################
# XXX From kantapon's studies, this documentation does not make any use for MTData2 at all. XXX
def usage():
...
#					
				...
				print 'after config'
				mt.ReqOutputConfiguration()
				
				
				for i in range(50): 
					output = mt.read_measurement2(dataLength)
#					output.keyout_acc = output['Acceleration']
#					out_tem = output['Temperature']
