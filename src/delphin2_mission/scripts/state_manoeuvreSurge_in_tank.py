...
class manoeuvreSurge(smach.State):
    def __init__(self, lib, myUti, errHeadingTol, timeDelay, timeDemandHold, depthDemand, depthTol, depthDemandMin, controlRate):
        smach.State.__init__(self, outcomes=['succeeded','aborted','preempted'])
        self.__controller = lib
        self.__uti = myUti
        self.__errHeadingTol = errHeadingTol
        self.__timeDelay = timeDelay # [sec]. to let the motion decays
        self.__timeDemandHold = timeDemandHold # hold the propeller demand for this many seconds
        ...
        self.__demandProp = 10 # propeller demand used in experiment
        self.__headingDemand = 90 # [deg]
    ...
    def execute(self, userdata):
        ...
        # set heading
        str = 'set heading to: %s' %self.__headingDemand
        rospy.loginfo(str)
        ...
        while not rospy.is_shutdown() and self.__controller.getBackSeatErrorFlag() == 0:
            ...
            errHeading = self.__uti.computeHeadingError(self.__headingDemand,heading)
            if abs(errHeading)>self.__errHeadingTol:
                timeStart = time.time()
            if time.time()-timeStart <= self.__timeDelay:
                self.__controller.setHeading(self.__headingDemand)
                if flag:
        ...
        # set demandProp
        str = 'apply propeller demand = %s' %(self.__demandProp)    
        rospy.loginfo(str)
        pubMissionLog.publish(str)
        timeStartProp = time.time()
        while not rospy.is_shutdown() and self.__controller.getBackSeatErrorFlag() == 0:
            ...
            if time.time()-timeStartProp < self.__timeDemandHold:
                self.__controller.setRearProp(self.__demandProp)
                if self.__depthDemand>=self.__depthDemandMin:
                    self.__controller.setDepth(self.__depthDemand)
            else:
