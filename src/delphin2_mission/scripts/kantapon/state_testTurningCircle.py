...
class testTurningCircle(smach.State):
    ...
    def execute(self, userdata):
        ...
        listThruster = [700,1400,2100] # list of thruster demand used in experiment
        listRudder = [10,20,30] # list of rudder angle used in experiment
        headingBias = 30 # [deg]
            ...
            for demandProp in listProp:
                for demandThruster in listThruster:
                    for demandRudder in listRudder:
                        ...
                        print 'rudder demand is ', demandRudder
                        # go to the waypoint
                        ...
                        while True:
                            ...
                            if rang < self.__wp_R:
                                ...
                                # TODO: add mechanism to get the AUV stay still
                        ...
                        X = self.__controller.getX()
                        Y = self.__controller.getY()                        
                        _, headingRef = self.__uti.rangeBearing([X,Y], [self.__wp[0][1-wpIndex], self.__wp[1][1-wpIndex]])
                        while True:
                            X = self.__controller.getX()
                            Y = self.__controller.getY()
                            heading = self.__controller.getHeading()
                            ...
                            self.__controller.setHeading(headingRef)
                            if rang > wpRang*0.75:
                                self.__controller.setRearProp(0)
                                break
