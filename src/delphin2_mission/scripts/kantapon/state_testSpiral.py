...
class testTurningCircle(smach.State):
    def __init__(self, lib, myUti, wp, uGain, uMax, errHeadingTol, wp_R, timeDemandHold, timeDelay, depthDemand, depthTol, depthDemandMin, turningAngle):
        ...
        self.__listProp = [10,16,22] # list of propeller demand used in experiment
        self.__listThruster = [0,800,1500,2200] # TODO [0,800,1500,2000] list of thruster demand used in experiment [+ve yaw CW]
        self.__listRudder = [0,10,20,30] # TODO [0,10,20,30]] list of rudder angle used in experiment [+ve yaw CW]
        self.__headingBias = -25 # TODO [deg, +ve CW] point away from the target just so the vehicle has enough space for turning
                ...
                for demandRudder in self.__listRudder:
                    ...
                    if demandRudder!=0 or demandThruster!=0: # if both actuator commands are zero, skip this combination
                        ...
                        while not rospy.is_shutdown():
                            ...
                            heading_accu = heading_accu+headingDis
                            headingOld = headingNow
                            # apply the demand
                            if abs(headingDisplace)<self.__turningAngle;
